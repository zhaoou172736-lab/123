...
<head>
    ...
    <title>AI 爆款视频拆解助手 (Powered by Gemini)</title>
...
<body class="min-h-screen flex flex-col relative">
        ...
        <div class="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8">
            <div class="flex justify-between h-16">
                <div class="flex items-center space-x-2">
                    ...
                    <span class="text-xl font-bold text-stone-800">爆款拆解助手 <span class="text-xs text-white bg-gradient-to-r from-blue-500 to-purple-600 px-2 py-0.5 rounded-full ml-2 shadow-sm">AI Core</span></span>
                </div>
                <!-- Settings Button -->
                <div class="flex items-center">
                    <button onclick="toggleSettingsModal()" class="flex items-center text-stone-500 hover:text-stone-800 px-3 py-2 rounded-lg hover:bg-stone-100 transition duration-200" title="API 设置">
                        ...
                        <span class="text-sm font-medium">API 设置</span>
                    </button>
                </div>
            </div>
        </div>
    ...
    <main class="flex-grow container mx-auto px-4 py-8 sm:px-6 lg:px-8 max-w-6xl flex flex-col justify-center">
        ...
        <div class="flex flex-col gap-8 max-w-5xl mx-auto"> 
                ...
                <div class="bg-white p-6 rounded-2xl shadow-lg border border-stone-200 flex flex-col flex-1 transform transition hover:scale-[1.002] duration-300 relative z-10">
                    <div class="mb-2 flex-grow flex flex-col">
                        <label class="block text-base font-bold text-stone-800 mb-4 flex items-center justify-between">
                            ...
                            <span class="text-xs font-normal text-stone-400 bg-stone-100 px-2 py-1 rounded">无限制 | 自动运行</span>
                        </label>
                        ...
                        <div id="dropZone" class="drop-zone flex-grow rounded-2xl flex flex-col items-center justify-center cursor-pointer hover:bg-stone-50 relative overflow-hidden group min-h-[250px]">
                            ...
                            <div id="uploadPlaceholder" class="text-center p-6 transition-opacity duration-300">
                                ...
                                <p class="text-stone-600 font-medium text-lg">点击浏览 或 拖拽上传视频</p>
                                <p class="text-stone-400 text-sm mt-2">支持 MP4, WebM, MOV</p>
                            </div>
                            ...
                            <div id="videoPreviewContainer" class="hidden absolute inset-0 z-30 bg-black flex flex-col items-center justify-center w-full h-full">
                                ...
                                <div class="absolute top-0 left-0 w-full p-2 bg-gradient-to-b from-black/70 to-transparent flex justify-between items-start z-20">
                                    ...
                                    <button onclick="removeVideo(event)" class="bg-black/50 hover:bg-red-600 text-white p-2 rounded-full backdrop-blur-sm transition pointer-events-auto cursor-pointer" title="删除视频">
            ...
            <!-- Bottom Section: Output (可视化容器) -->
            <!-- 布局修改：改为独立的一行，高度固定 h-[600px] -->
    ...
    <div id="settingsModal" class="hidden fixed inset-0 bg-black/60 backdrop-blur-sm z-[100] flex items-center justify-center p-4">
        <div class="bg-white rounded-2xl shadow-2xl w-full max-w-lg mx-auto transform transition-all overflow-hidden">
            ...
            <div class="p-6 space-y-5">
                ...
                <div>
                    <label class="block text-xs font-bold text-stone-500 uppercase tracking-wide mb-2">服务商 (AI Provider)</label>
                ...
                <!-- 2. Model Selection -->
                <div>
                    <label class="block text-xs font-bold text-stone-500 uppercase tracking-wide mb-2">模型 (Model Name)</label>
                    <div class="relative">
                    ...
                    <!-- 更新后的模型推荐列表 (严格对应用户提供的功能注释) -->
                    <datalist id="model-suggestions">
                        <!-- Pro 系列 -->
                        <option value="gemini-2.5-pro">Gemini 2.5 Pro (支持6小时长视频/影视拆解/监控行为分析/多模态融合)</option>
                        <option value="gemini-2.5-pro-thinking">Gemini 2.5 Pro Thinking (长视频解析+思考模式/时序推理/剧情因果梳理)</option>
                        
                        <!-- 3.0 旗舰系列 -->
                        <option value="gemini-3-pro-preview">Gemini 3 Pro Preview (4K视频内容标注/工业质检/高分辨率解析/实时对齐)</option>
                        
                        <!-- 更新: 补充 Gemini 3 Pro Thinking -->
                        <option value="gemini-3-pro-preview-thinking">Gemini 3 Pro Thinking (旗舰级精度/高分辨率解析+增强时序推理/高精度行为识别)</option>

                        <!-- Flash 系列 -->
                        <option value="gemini-2.5-flash">Gemini 2.5 Flash (短视频解析≤1h/快速内容提取/摘要生成/低成本)</option>
                        <option value="gemini-2.5-flash-image">Gemini 2.5 Flash Image (视频转图像片段/截图提取/视觉特征标注)</option>
                        <option value="gemini-2.5-flash-thinking">Gemini 2.5 Flash Thinking (短视频解析+轻量时序推理/Vlog分类/剧情脉络)</option>
                        
                        <!-- 更新: 补充 Gemini 3 Flash Preview -->
                        <option value="gemini-3-flash-preview">Gemini 3 Flash Preview (3代轻量旗舰/延迟极低/直播内容实时摘要/短视频平台审核)</option>

                        <!-- 补回 OpenAI 官方模型 -->
                        <option value="gpt-4o">GPT-4o (OpenAI旗舰/综合能力强/复杂逻辑分析)</option>
                        <option value="gpt-4o-mini">GPT-4o Mini (OpenAI快速版/低成本/高性价比)</option>

                        <!-- 补充 DeepSeek 系列 (OpenAI 兼容) -->
                        <option value="deepseek-chat">DeepSeek V3 (国产之光/高性价比/通用能力强)</option>
                        <option value="deepseek-reasoner">DeepSeek R1 (深度推理/逻辑分析/数理推导)</option>
                    </datalist>
                </div>

                <!-- 3. Base URL -->
                <div>
                    <label class="block text-xs font-bold text-stone-500 uppercase tracking-wide mb-2">API 地址 (Base URL)</label>
                    <div class="relative">
                        <!-- Update: Removed icon and pr-10 padding -->
                    ...
                    <p id="baseUrlHint" class="text-xs text-stone-400 mt-1.5 ml-1">默认: https://generativelanguage.googleapis.com</p>
                </div>
                ...
                <div>
                    ...
                    <p class="text-xs text-stone-500 mt-1.5 ml-1">密钥仅保存在本地浏览器 LocalStorage 中</p>
                </div>
            </div>
    ...
    <script>
        ...
        let progressInterval = null; // 用于存储进度条定时器
        ...
        function switchProvider(provider, resetDefaults = true) {
            ...
            if (provider === 'gemini') {
                ...
                document.getElementById('baseUrlHint').innerText = `默认: ${CONFIG.gemini.defaultUrl}`;
            } else {
                ...
                document.getElementById('baseUrlHint').innerText = `默认: ${CONFIG.openai.defaultUrl}`;
            }
        ...
        function saveSettings() {
            ...
            alert(`✅ ${currentProvider === 'gemini' ? 'Gemini' : 'OpenAI'} 模式配置已保存！`);
        }
        ...
        function resetMetaCard() {
            ...
            if(audienceEl) audienceEl.innerHTML = '<span class="text-stone-300 italic">等待分析...</span>';
            if(topicEl) topicEl.innerHTML = '<span class="text-stone-400 font-normal italic">等待视频分析...</span>';
            if(tagsEl) tagsEl.innerHTML = '<span class="bg-stone-100 text-stone-300 px-2 py-1 rounded text-xs">#等待分析</span> <span class="bg-stone-100 text-stone-300 px-2 py-1 rounded text-xs">#AI识别</span>';
        }
        ...
        function callAnalysis() {
            ...
            第1行：必须是 JSON 元数据注释，格式如下：
            <!-- META: {"topic": "这里填运用了反差/痛点/场景生成的核心选题标题", "audience": "这里填目标人群", "tags": ["领域标签", "情绪标签", "结果标签", "细分标签"]} -->
            ...
            document.getElementById('metaTopic').innerHTML = '<span class="text-orange-500 animate-pulse">正在运用算法生成...</span>';
            document.getElementById('metaTags').innerHTML = '<span class="bg-stone-100 text-stone-300 px-2 py-1 rounded text-xs animate-pulse">#算法匹配中...</span>';
        }
        ...
        function updateHeaderWithInsight(fullText) {
            ...
            if (match && match[1]) {
                ...
                setTimeout(() => {
                    headerIntro.innerHTML = `🚀 <span class="font-bold text-orange-600 text-xl">${insight}</span>`;
        ...
        // 🌟 新增功能：解析并更新左侧的 Meta 分析卡片 (选题 & 标签 & 人群)
        ...
        async function callGeminiImpl(systemPrompt, loadingMsg) {
            const outputDiv = document.getElementById('aiOutput'); // 已经是 Div 了
            ...
            loadingText.innerText = loadingMsg || "Gemini 正在思考...";
            loader.classList.remove('hidden');
            // outputDiv.innerHTML = ""; // 可以不清空，或者显示骨架屏，这里先保留原状
            ...
            try {
                ...
                if (data.candidates && data.candidates[0].content) {
                    ...
                    setTimeout(() => {
                        ...
                        renderReport(resultText); // 使用 HTML 渲染
                    }, 500);
                } else {
                    throw new Error("Gemini API 返回数据格式异常");
                }
            } catch (error) {
                if (progressInterval) clearInterval(progressInterval);
                loader.classList.add('hidden');
                outputDiv.innerText = `❌ 出错: ${error.message}`;
            }
        }
        ...
        async function callOpenAIImpl(systemPrompt, loadingMsg) {
            ...
            loadingText.innerText = loadingMsg || "AI 正在分析...";
            loader.classList.remove('hidden');
            // outputDiv.innerHTML = "";
        ...
        function checkAuth(apiKey, baseUrl, defaultUrl) {
            ...
            if (!apiKey && baseUrl === defaultUrl) {
                loader.classList.add('hidden');
                const userConfirmed = confirm("⚠️ 未检测到 API Key。请点击右上角⚙️配置。是否继续尝试 (可能失败)？");
        ...
        function handleError(error, outputElement) {
            // 简单的错误展示
            outputElement.innerHTML = `<div class="text-red-400 p-4 border border-red-500 rounded">❌ 请求失败: ${error.message}</div>`;
        }
        ...
        function handleFileSelect(input) {
            ...
            loadingText.innerText = `正在读取视频数据 (${(file.size / 1024 / 1024).toFixed(1)} MB)... 请稍候`;
            loader.classList.remove('hidden');
            ...
            setTimeout(() => {
                ...
                reader.onloadend = function() {
                    ...
                    loader.classList.add('hidden');
                    loadingText.innerText = "正在连接大模型分析画面，请稍候..."; 
                }
        ...
        function removeVideo(e) {
            ...
            // 恢复顶部文案
            const headerIntro = document.getElementById('header-intro');
            headerIntro.innerHTML = `直接上传爆款视频文件，AI 将自动分析并揭秘其<span class="font-bold text-stone-700">“画面、脚本与情绪”</span>背后的流量密码。`;
